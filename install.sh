...
upload_file() {
    ...
    if [ "$FILE" = "admin.html" ]; then
        ...
        CONTENT="${CONTENT//let repo = \"Ziploot.github.io\";/let repo = \"$REPO_NAME\"}"
    fi
    if [ "$FILE" = "404.html" ]; then
        CONTENT="${CONTENT//const owner = \"Ziploot\";/const owner = \"$USERNAME\"}"
        CONTENT="${CONTENT//const repo = \"Ziploot.github.io\";/const repo = \"$REPO_NAME\"}"
    fi
